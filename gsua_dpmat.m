...
addParameter(p,'EAE',dfEAE,chEAE);
...
if size(domain,2)==1
    ...
    if ~isempty(EAE)
        ...
        related=[1,2,9,10];
        solver= @(init,pars) funEAE(odefun,init,pars,EAE,related,domain,opt,extraPars,sname);%
    else
...
try
    ...
    Table.Properties.CustomProperties.Kind='mat';
    Table.Properties.CustomProperties.Vars=RNames(:,1)';
    Table.Properties.CustomProperties.Fixed=[];
    Table.Properties.CustomProperties.Sname=sname;
...
function w=funEAE(odefun,init,pars,odes,related,domain,opt,extraPars,sname)
pars([4,5,20,21])=round(pars([4,5,20,21]));
odes=subs(odes,extraPars,pars);
vars=symvar(odes);

odes=subs(odes,vars([2,3,4,5]),init([10,1,2,9]));
vars=symvar(odes);

 for i=1:size(vars,2)
     assume(vars(i)>=0);
 end
Y=solve([odes(1:6)==0],vars);
Y=double([Y.E,Y.L,Y.P,Y.Ms,Y.Me,Y.Mi]);
if size(Y,1)>1
    Y=Y(Y(:,1)>0,:);
end
Y=[init(1:2),Y,init(9:10)];
if strcmp(sname,'ode45')
    w=ode45(@(t,X) odefun(t,X,pars),domain,Y,opt);
else
    w=ode4(@(t,X) odefun(t,X,pars),domain,[],Y,opt);
end
end